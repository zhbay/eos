...
producer_plugin_impl::start_block_result producer_plugin_impl::start_block(bool &last_block) {
   //elog( "producer_plugin_impl start_block " );
   chain::controller& chain = app().get_plugin<chain_plugin>().chain();
...
void producer_plugin_impl::schedule_production_loop() {
  ...
  // elog( "producer_plugin_impl schedule_production_loop  loopid=${id}" ,("id",loopcount));
   chain::controller& chain = app().get_plugin<chain_plugin>().chain();
   _timer.cancel();
   std::weak_ptr<producer_plugin_impl> weak_this = shared_from_this();

   bool last_block;
  // elog( "producer_plugin_impl schedule_production_loop->start_block  loopid=${id}" ,("id",loopcount));
   ...
   if (result == start_block_result::failed) {
      ...
      _timer.async_wait([&chain,weak_this,cid=++_timer_corelation_id](const boost::system::error_code& ec) {
         ...
         if (self && ec != boost::asio::error::operation_aborted && cid == self->_timer_corelation_id) {
            ...
            //elog( "producer_plugin_impl schedule_production_loop->maybe_produce_block loopid=${id}",("id",loopcount) );
...
bool producer_plugin_impl::maybe_produce_block() {
  // elog( "producer_plugin_impl maybe_produce_block loopid=${id}",("id",loopcount) );
   auto reschedule = fc::make_scoped_exit([this]{
      //elog( "producer_plugin_impl maybe_produce_block->schedule_production_loop  loopid=${id}" ,("id",loopcount));
      schedule_production_loop();
   });

   try {
     // elog( "producer_plugin_impl maybe_produce_block->produce_block loopid=${id}" ,("id",loopcount));
      produce_block();
      return true;
   } catch ( const guard_exception& e ) {
      app().get_plugin<chain_plugin>().handle_guard_exception(e);
